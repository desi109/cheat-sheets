...
![Popular Distributed Message Queues](images/20-distributed-message-queue/figure1-popular-distributed-message-queues.png)
...
![Key Components in a Message Queue](images/20-distributed-message-queue/figure2-key-components-in-a-message-queue.svg)
...
3. Message queue is a service in the middle that decouples the producers from the consumers, allowing each of them to operate and scale independently.
...
![Point-to-Point Model](images/20-distributed-message-queue/figure3-point-to-point-model.svg)
...
![Publish-Subscribe Model](images/20-distributed-message-queue/figure4-publish-subscribe-model.svg)
...
![Partitions](images/20-distributed-message-queue/figure5-partitions.svg)
...
![Message Queue Cluster](images/20-distributed-message-queue/figure6-message-queue-cluster.svg)
...
![Consumer Groups](images/20-distributed-message-queue/figure7-consumer-groups.svg)
...
![High-Level Design](images/20-distributed-message-queue/figure8-high-level-design.svg)
...
1. We chose an on-disk data structure that takes advantage of the great sequential access performance of rotational disks and the aggressive disk caching strategy of modern operating systems.
2. We designed the message data structure to allow a message to be passed from the producer to the queue and finally to the consumer, with no modifications. This minimizes the need for copying which is very expensive in a high volume and high traffic system.
3. We designed the system to favor batching. Small I/O is an enemy of high throughput. So, wherever possible, our design encourages batching. The producers send messages in batches. The message queue persists messages in even larger batches. The consumers fetch messages in batches when possible, too.
...
![Append New Messages](images/20-distributed-message-queue/figure9-append-new-messages.svg)
...
![Data Segment File Distribution](images/20-distributed-message-queue/figure10-data-segment-file-distribution.svg)
...
![Routing Layer](images/20-distributed-message-queue/figure11-routing-layer.svg)
...
![Producer with Buffer and Routing](images/20-distributed-message-queue/figure12-producer-with-buffer-and-routing.svg)
...
![The Choice of the Batch Size](images/20-distributed-message-queue/figure13-choice-of-batch-size.svg)
...
![Consumer Flow](images/20-distributed-message-queue/figure14-consumer-flow.svg)
...
![Pull Model](images/20-distributed-message-queue/figure15-pull-model.svg)
...
![Coordinator of Consumer Groups](images/20-distributed-message-queue/figure16-coordinator-of-consumer-groups.svg)
...
1. The coordinator maintains a joined consumer list. When the list changes, the coordinator elects a new leader of the group.
2. As the new leader of the consumer group, it generates a new partition dispatch plan and reports it back to the coordinator. The coordinator will broadcast the plan to the other consumers in the group.
...
![Consumer Rebalance](images/20-distributed-message-queue/figure17-consumer-rebalance.svg)
...
![New Consumer Joins](images/20-distributed-message-queue/figure18-new-consumer-joins.svg)
...
![Existing Consumer Leaves](images/20-distributed-message-queue/figure19-existing-consumer-leaves.svg)
...
![Existing Consumer Crashes](images/20-distributed-message-queue/figure20-existing-consumer-crashes.svg)
...
![Last Consumed Offset of Consumer Groups](images/20-distributed-message-queue/figure21-last-consumed-offset.svg)
...
![Zookeeper](images/20-distributed-message-queue/figure22-zookeeper.svg)
...
![Replication](images/20-distributed-message-queue/figure23-replication.svg)
...
1. The committed offset in the leader replica is 13. Two new messages are written to the leader, but not committed yet. Committed offset means that all messages before and at this offset are already synchronized to all the replicas in ISR.
2. Replica-2 and replica-3 have fully caught up with the leader, so they are in ISR and can fetch new messages.
3. Replica-4 did not fully catch up with the leader within the configured lag time, so it is not in ISR. When it catches up again, it can be added to ISR.

![How ISR Works](images/20-distributed-message-queue/figure24-how-isr-works.svg)
...
![ack=all](images/20-distributed-message-queue/figure25-ack-all.svg)
...
![ack=1](images/20-distributed-message-queue/figure26-ack-1.svg)
...
![ack=0](images/20-distributed-message-queue/figure27-ack-0.svg)
...
![Broker Node Crashes](images/20-distributed-message-queue/figure28-broker-node-crashes.svg)
...
![Add New Broker Node](images/20-distributed-message-queue/figure29-add-new-broker-node.svg)
...
![Partition Increase](images/20-distributed-message-queue/figure30-partition-increase.svg)
...
![Partition Decrease](images/20-distributed-message-queue/figure31-partition-decrease.svg)
...
![At-Most Once](images/20-distributed-message-queue/figure32-at-most-once.svg)
...
![At-Least Once](images/20-distributed-message-queue/figure33-at-least-once.svg)
...
![Exactly Once](images/20-distributed-message-queue/figure34-exactly-once.svg)
...
![Message Filtering by Tags](images/20-distributed-message-queue/figure35-message-filtering-by-tags.svg)
...
![Delayed Messages](images/20-distributed-message-queue/figure36-delayed-messages.svg)
